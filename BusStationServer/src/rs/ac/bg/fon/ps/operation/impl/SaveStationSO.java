 ...
 */
public class SaveStationSO extends AbstractSO {

    public SaveStationSO(TransferObject tObj) {
        super(tObj);
    }
    ...
    protected boolean preconditions() throws Exception {
        try {
            ...
            if (!station.getState().equals(State.NEW)) {
                throw new ConstraintException(station.message17());
            }
            return true;
        } catch (ConstraintException cex) {
            StringBuilder sb = new StringBuilder();
            sb.append(new Station().message6()).append(":\nPreconditions not satisfied:")
    ...
    protected void executeSO() throws Exception {
        ...
        Station station = (Station) tObj.getGdo();
        try {
            signal = repository.insertRecord(station);
        } catch (Exception e) {
            throw e;
        } finally {
            tObj.setSignal(signal);
            if (signal) {
                tObj.setMessage(station.message5());
            } else {
                tObj.setMessage(station.message6());
            }
        }
    }
